...
import AboutView      from '../views/Task3/AboutView.vue'
...
router.beforeEach((to, from, next) => {
  ...
  if (to.meta.requiresTask) {
    ...
    const taskExists = !!taskStore.getTaskById(taskId)

    if (!taskExists) {
      next({ path: '/home', query: { error: 'notfound' } })
      return // important to return here to prevent calling next() twice
    }
  } 
